...
# ------------------------------------------------------------------------------------------------------------------------------
    if not cfg['label'] == ['volumetric_soil_water_layer_20'] and cfg['modelname'] in ['LSTM','TimeNetModel']:
        print('2 ---------------------------------')
        ...
        for i in range(nlat):
            for j in range(nlon):
                # if not (np.isnan(y_test_lstm[:, i, j]).any()) and not (np.isfinite(y_test_lstm[:, i, j]).all()):
                if not (np.isnan(y_test_lstm[:, i, j]).any()):
                    ...
                    # print(y_test_lstm[40, i, j],y_pred_lstm[40, i, j],'----------------',i,j)
                    # if i==19 and j==286:
                    #     print(y_test_lstm[:, i, j],y_pred_lstm[:, i, j],'----------------')
                    #     break
                    r_lstm[i, j] = np.corrcoef(y_test_lstm[:, i, j], y_pred_lstm[:, i, j])[0,1]
